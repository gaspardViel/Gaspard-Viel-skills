...
### 2. Compute the Suivi de complétude
...
### 3. Compute which Relances are due
...
- **`à corriger` Livrable** — its deadline is the **resubmission date**, and the cadence runs a **fresh cycle** against it (prior log entries don't block the new ticks).
- **Ressource** — never dated-due. It appears only in the step-2 completeness report, never as a scheduled Relance (ADR-0006).
...
### 4. Draft each due Relance, targeting by state
...
- **Ressource → inward.** Its nudge *is* the step-2 completeness line ("still missing X"); no separate message, no gate.
...
Append every Relance that fired a dated tick to that element's log in the **Manifeste** — both the **outward** send and the **inward** `à corriger` nudge. This is what makes step 3 idempotent: an unlogged tick would re-propose itself on the next run. A Ressource line in the report is not a dated tick and is not logged.
